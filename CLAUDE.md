...
| `tabslate-analytics-id` | 随机字符串（`generateId()`） | 匿名分析会话 ID；首次使用时由 `lib/analytics.ts` 生成并持久化；不含任何可识别个人信息，不与账号关联 |

适配器：`lib/auth-storage-adapter.ts`（透明合并两个 storage 供 Zustand `persist` 中间件使用；含从旧 `accessToken`-in-local 的迁移路径）。

#### IndexedDB — `tabslate-db` v3（`lib/idb.ts`）

v2 迁移（`oldVersion >= 1 && < 2`）：为 `trashed-bookmarks` 添加 `collectionId` 索引，支持按集合查询回收站书签。新安装直接在 v1 建表时包含此索引。

v3 迁移（`oldVersion < 3`）：为 `groups` 添加 `workspaceId` 索引，供 workspace aggregate 发现（`lib/workspace-aggregate.ts`）按工作区批量定位 saved group。
...
| `tags` | `id` | — | 标签 |
| `groups` | `id` | `workspaceId` | 保存的标签组 |
| `group-tabs` | `id` | `groupId` | 保存组内的 tab |
| `tab-group-titles` | `groupId` | — | Chrome tab group 完整标题（compact 模式下 Chrome 侧只存首字母）；keyed by ephemeral Chrome group ID，重启后由 `loadTabs` 通过两层恢复逻辑重新关联 |
| `kv` | `key` | — | 键值对：`activeWorkspaceId`、`compactGroupTitles`、`localSeq`、`sync-leader`、`searchEngines`、`guest-workspace-provenance-v1`（自动 guest seed 的 workspace/default collection 身份）、`sync-conflicts-v1`（持久化同步冲突）、`workspace-lifecycle-intents-v1`（待确认的 workspace 生命周期意图）、`workspace-lifecycle-deferred-sync-v1`（按 workspace 隔离的延迟推送快照）、`guest-workspace-orphan-recovery-v1`（legacy guest 孤儿聚合恢复标记）；按 `origin + userId` 作用域的 `workspace-parent-tombstone-capability-v1:*` 与 `workspace-parent-tombstone-full-pull-v1:*`；以及 compact group 稳定标题条目 `compact-group-title:${color}:${firstChar}`（详见 Compact group title 注意事项） |
...
项目使用原生的 Chrome i18n 配合 Zustand (`store/i18n-store.ts`) 和自定义 hook (`hooks/use-translation.ts`) 实现多语言。

`resolveAcceptLanguage(lang: SupportedLanguage): string`（`store/i18n-store.ts` 导出）：将 store 中的语言设置映射为 HTTP `Accept-Language` 值（`"zh_CN" → "zh-CN"`，`"en" → "en"`，`"auto" → navigator.language`）。auth-store 的 4 个 OTP 触发方法在请求前调用此函数。

1. **基本用法**：
   在组件中导入 hook，使用 `t` 函数读取：
   ```tsx
   import { useTranslation } from "@/hooks/use-translation";
   
   export function MyComponent() {
     const { t } = useTranslation();
     return <div>{t("myComponent_title")}</div>;
   }
   ```
2. **复数与占位符**：
   使用带有 `$1` 的占位符，并在 `messages.json` 中定义 `placeholders`。在组件中通过数组传参：
   ```tsx
   {t(count === 1 ? "key_one" : "key_other", [count.toString()])}
   ```
3. **语言切换与持久化**：
   用户选择的语言保存在 `chrome.storage.local` 的 `tabslate-i18n` 中。由于 `chrome.i18n.getMessage` 只在扩展启动时根据浏览器语言静态求值，所以用户主动切换语言时，我们通过 `i18nStore` 中缓存的热重载语言包来覆盖原生 API。开发者**必须**使用 `useTranslation` 的 `t` 方法，不要直接调用 `chrome.i18n.getMessage`。
4. **命名规范**：
   在 `messages.json` 中的 key 应保持 `[组件名或模块名]_[描述]` 的驼峰格式，如 `sidebar_allBookmarks`、`trashContent_emptyTrash`。
...
| `VITE_OPENPANEL_URL` | 自部署 OpenPanel 实例地址（如 `https://analytics.example.com`）；空 = 禁用分析 |
| `VITE_OPENPANEL_CLIENT_ID` | OpenPanel 客户端 ID；空 = 禁用分析 |
...
- **Store hydration**：`App.tsx` 中的 `StoreGate` 等待 bookmarks、workspace、auth、groups、settings 五个 store 全部 hydrate 后再渲染 dashboard；guest 与已认证用户走相同的 dashboard 路由，不再使用全屏认证守卫。若 hydrate 完成后会话是 guest 且本地没有 workspace，`StoreGate` 创建一个含 Default collection 的空 guest workspace。guest 创建的 workspace、collection、bookmark 等实体保持 `seq === 0` 并持久化在 IndexedDB；用户完成邮箱验证后 Verified-only `SyncEngine` 启动，通过首次 enqueue 或 `sweepUnsynced()` 将这些本地实体并入账号。
- **认证生命周期与 AuthDialog**（`entrypoints/newtab/App.tsx`、`components/auth/auth-dialog.tsx`）：guest 始终可使用 dashboard；Sidebar 顶部 “Not logged in” / “未登录” 用户卡片以 login mode 打开 `AuthDialog`，底部同名同步状态区域的 “Register now” 入口以 register mode 打开。`LoginForm` 在同一 Dialog 内切换 login / register / forgot-password / reset-password，不离开 dashboard。若已登录用户 `user.is_verified === false`，同一 `AuthDialog` 强制切换为 `VerifyEmailScreen`；此时隐藏关闭按钮并阻止 X、Escape、outside click，只有验证成功或 “use different account” 完成 logout 后才能离开。浏览器重启时 `accessToken` 因 session storage 清除而为 null、但 verified user 与 `refreshToken` 仍在时，dashboard 保持显示并由 `silentRefresh` 后台恢复 token；临时网络失败保留用户资料并显示 Offline。显式 logout 或 refresh 返回 401/403 时，auth store 先 `clearDB()` 并清除账号数据，再转为 guest；`StoreGate` 随后 reset 各账号 store，并在空库中初始化全新的 guest workspace。
- **API 客户端**：`lib/api.ts` 是纯函数 HTTP 客户端，不持有状态；server URL 由 `useAuthStore.serverUrl` 管理（默认读取 `VITE_API_URL` 环境变量）。自托管用户可在登录页"Advanced"中修改。`searchBookmarks(serverUrl, accessToken, query)` 调用 `GET /search?q=`，最少 2 个字符。`register`/`login`/`resendVerification`/`forgotPassword` 接受可选 `lang?: string`，非空时注入 `Accept-Language` 请求头，供服务端发送本地化 OTP 邮件。
...
- **可选权限与动态注入**：为了规避安装时的“读取并更改所有网站数据”警告，`<all_urls>` 被配置为 `optional_host_permissions`。
    - **wxt.config.ts**：通过 `hooks.build:manifestGenerated` 钩子强制删除 WXT 自动生成的 `host_permissions`，并在 `VITE_OPENPANEL_URL` 非空时将其 origin 写入 `host_permissions`（供扩展页面免 CORS 直接 fetch 分析接口）。
...
- **邮箱验证**：使用 6 位数字 OTP，不再发送链接。注册或登录后若 `user.is_verified === false`，`AuthDialog` 强制显示不可关闭的 `VerifyEmailScreen`，用户输入 OTP 后调用 `POST /auth/verify-email { email, code }`，成功后通过 `GET /auth/me` 确认验证状态并自动关闭 Dialog。`VerifyEmailScreen` 挂载时从 store 的 `otpSentAt` 字段本地计算剩余冷却时间（不发 API 请求），同时调 `GET /auth/otp-captcha-status` 检查 IP 是否需要验证码。`InputOTP` 的 `onChange` 在 `value.length === 6` 时立即调 `submitCode`，无需用户点击"Verify"按钮（自动提交）。
...
- **Compact group title**：Chrome tab group 标题若为紧凑模式，Chrome 端只存首字母（如 `"W"`）；完整标题通过 `useTabsStore` 的 `fullTitles: Record<number, string>` 在内存中维护，同时持久化到 IDB `tab-group-titles`（keyPath = `groupId`）。**Chrome tab group ID 是短暂的**——每次浏览器重启 ID 都会变化，导致 IDB 中旧 ID 的记录成为"孤儿"。`loadTabs` 通过两层恢复逻辑重新关联完整标题：
  - **Layer 1（孤儿条目对账）**：对每个当前 compact group（`title.length === 1` 且无 IDB 匹配），在孤儿条目中按 `(firstChar === group.title && title.length > 1)` 查找候选——`length > 1` 过滤旧版 bug 写入的单字母污染条目；仅当候选唯一时才匹配并回写新 ID。
  - **Layer 2（稳定 kv 回退）**：`toggleGroupCompact` 启用 compact 时同时向 `kv["compact-group-title:${color}:${firstChar}"]` 写入完整标题——`color` 和 `firstChar` 均不随重启改变。Layer 1 失败时（多个同首字母孤儿、IDB 已清空等）由此兜底。关闭 compact 或 dissolve/close group 时删除对应 kv 条目。
  - **`isCurrentlyCompact` 判断**：仅用 `group.title.length === 1`，不依赖 `fullTitles[groupId]`——全标题丢失时后者退化为单字母，旧写法导致重启后按钮无法点击。
  - **无名称分组**：`displayTitle` 为空时，点击 Compact 切换显示 `tabsPanel_compactUnnamedAlert` 提示，不执行 `toggleGroupCompact`。
- **SyncEngine 生命周期**：`SyncProvider`（`App.tsx`）仅在会话状态为 verified（`user.is_verified === true`、存在 `accessToken` 且 `serverUrl` 非空）时创建并启动引擎；guest、unverified 与只有 verified user + `refreshToken` 的 offline 会话均不启动引擎，offline 会话仍保留 dashboard 与用户资料并把有效同步状态显示为 Offline。verified gate 或 `serverUrl` 变更时 cleanup 依次调用 `engine.forceSync()`（fire-and-forget）、`engine.destroy()`、`releaseSyncEngine(engine)`。**禁止在 cleanup 中调用 `destroySyncEngine()`**——异步结束时新引擎可能已写入全局变量，`destroySyncEngine()` 会错误销毁新引擎。`releaseSyncEngine(engine: SyncEngine)` 位于 `lib/sync-engine.ts`，仅在全局指针仍指向同一实例时清空，保证安全。不要在组件中直接 `import syncEngine`——通过 `SyncProvider` render props 的 `onForceSync` 触发手动同步。`SyncProvider` render props 签名为 `(syncStatus, onForceSync, syncErrorMessage)`；`syncErrorMessage` 在 `status === "error"` 时携带错误原因文本，其他状态下为 `null`。`SyncStatusIndicator` 在 error 状态下悬停时通过 Tooltip 展示该错误原因。push 收到 `quota_exceeded` 拒绝时，`onPushSuccess` 从 `Rejected.type` 字段（`"collection"` | `"saved_group"` | `"workspace"`）读取资源类型，调用 `showQuotaAlert(type)` 并触发 `fetchPlan()`，不将 sync status 设为 error。`SyncEngine.OnStatusChange` 类型为 `(status, errorMessage?) => void`；push 网络错误和 pull catch 分别传入对应错误消息。
- **syncEngine?.enqueue() 调用位置**：必须在 Zustand `set()` 调用之前（或完全在外部），不能在 `set((state) => {...})` updater 函数内部调用（updater 必须是纯函数）。
- **guest workspace 恢复与 `localSeq`**：自动创建的 guest workspace 会将其 workspace/default collection 身份写入 `kv["guest-workspace-provenance-v1"]`。首次 pull 在合并服务端实体前协调：未改动的自动 seed 遇到已有账号 workspace 时丢弃；有内容的 guest workspace 在有套餐容量时迁移到确定的已确认 workspace；无可用目标或容量时保留本地并记录冲突。`App.tsx` 的 `onPullSuccess` 回调是唯一更新 `localSeq` 的地方；它必须在协调和三个 store 合并完成后写入 seq，再进行冲突感知的 `sweepUnsynced()`。`sync-conflicts-v1` 中被阻断的实体不会被自动 sweep；用户手动同步会清除冲突并重试，套餐容量释放后也会清除可恢复的 quota 根节点再补推。`bookmarks-store.sweepUnsynced` 若 archived/trashed 尚未加载，会先调 `loadArchivedBookmarks()` / `loadTrashedBookmarks()` 加载并保留在 state（load-and-keep），避免读取并丢弃产生 GC 峰值。
...
- **软删除 / 归档（集合三态）**：集合有三种状态：active（`!deletedAt && !archivedAt`）、archived（`archivedAt && !deletedAt`）、trashed（`deletedAt`）。`deleteCollection` / `archiveCollection` 均为软删除——只写 `deletedAt` / `archivedAt` 并 `idbPut`，不做 `idbDelete`；`permanentlyDeleteCollection` 才真正 `idbDelete`。**`permanentlyDeleteCollection` 采用异步 push-first 模式**：先乐观移除 UI（从 `collections` state 过滤），调用 `syncEngine.forcePush({ collections: [...] })` 直接推送 `is_deleted: 2` 到服务端（非防抖队列）；push 失败时回滚 UI，push 成功后才执行 `idbDelete` + `decrementUsage("collection")`，确保 IDB 与服务端始终一致。删除/归档集合时，其下所有书签同步批量移入 `trashed-bookmarks` / `archived-bookmarks`（`trashCollectionBookmarks` / `archiveCollectionBookmarks`）；`restoreCollection` 时全部还原。**`trashCollectionBookmarks(id)` 同时覆盖 `bookmarks`（活跃）和 `archivedBookmarks`（已归档）两个 bucket**——若集合内有已归档书签，也会一并移入 `trashedBookmarks`，避免孤儿归档。`permanentlyDeleteCollectionBookmarks(id)` 不再 forcePush 书签 tombstone（服务端在处理集合 `is_deleted=2` 时已级联 `UPDATE bookmarks SET is_trashed=2`），仅执行本地 IDB 清理（同时覆盖 `trashed-bookmarks` 和 `archived-bookmarks`）。**`deleteWorkspace` 已改为 parent tombstone（父级墓碑）模型，不再级联删除子实体**——详见下方「Workspace 三态生命周期」。**`reassignCollection(fromId, toId)`** 是独立的工具方法，将 `bookmarks`（活跃）中属于 `fromId` 的书签批量改写为 `toId`；适用于集合合并等场景，与 trash/archive 生命周期操作不冲突。`mergeFromServer` 中本地待确认墓碑（`seq === 0 && (deletedAt || archivedAt)`）优先于服务端的存活状态（跳过服务端值），解决重登录后集合重新出现的 bug；`sweepUnsynced` 会在下次同步时将 `seq=0` 的墓碑补推给服务器。集合的 `isDefault` 由服务端 pull 响应携带（字段 `sc.is_default`，服务端 CTE 计算：每个 workspace 中 position 最小的活跃集合为 `true`）；`workspace-store.mergeFromServer` 直接读取 `sc.is_default` 写入 state，不再本地修复。离线/首次 hydrate 时若无 `isDefault=true` 的集合，`hydrate()` 用轻量 fallback 将最低 position 活跃集合临时标记为默认，下次 pull 后被服务端确认值覆盖。服务端集合配额检查条件为 `is_deleted < 2`：活跃、归档和回收站都占配额，只有永久删除释放配额。**`mergeFromServer` 对服务端确认删除的集合必须 update+keep（不可从 `collections` 数组移除）**：`getTrashedCollections()` 依赖 `collections.filter(c => !!c.deletedAt)` 找到回收站集合，移除后回收站在同步后失去集合分组，退化为单个书签列表。**`mergeFromServer` collections 的 `sc.is_deleted === 2` 检查是第一个条件**：state=2 记录直接 `idbDelete("collections", sc.id)` 并跳过所有合并逻辑，永不写入本地 state 或 IDB。**恢复回收站书签（`restoreFromTrash`）三级优先**：`restoreFromTrash(bookmarkId, collectionIdOverride?)` 接受可选目标集合 ID；`TrashedBookmarkCard.handleRestore` 按以下顺序解析：① 原 `collectionId` 仍为活跃集合 → 直接使用；② 同名活跃集合存在 → 使用同名集合；③ 回退到 default 集合。此逻辑在 UI 层（`trash-content.tsx`）而非 store 层实现，避免 `bookmarks-store` 反向依赖 `workspace-store`（循环依赖）。
- **Workspace 三态生命周期（parent tombstone）**：workspace 有三态——`0` 活跃、`1` 已删除但可恢复（根被打上墓碑，**子实体保持原样不动**，仅被隐藏）、`2` 终态（服务端保留一条被擦除的防复活记录，子实体物理删除）。服务端是生命周期权威；protocol version 2 通过 `lifecycle_action`（`delete` / `restore` / `purge`）显式转换，普通 metadata 变更**不能**改变生命周期状态。要点：
  - `deleteWorkspace` 只在**单个 IDB 事务**中写入根墓碑 + 生命周期意图 + 活跃工作区替换；不触碰任何 collection / bookmark / group，也**不释放配额**。
  - 只有终态 `2` 释放配额；`usage = in_use + trash` 对每种资源都成立。
  - `activeWorkspaceId` 在 bootstrap 后必须始终指向活跃 workspace；最后一个活跃 workspace 不可删除（前端与服务端事务内双重校验）。
  - 普通 UI / REST / 搜索**永不**暴露 retained 父级下的内容；Sync pull 仍会拉取（它是恢复通道）。
  - `components/dashboard/workspace-manager/` 是删除恢复与永久删除的**唯一**入口。
  - 生命周期意图在 SyncEngine 的串行 pull 边界内协调（`lib/workspace-lifecycle-coordinator.ts`）；意图未结算前不得刷新普通队列。
  - 破坏性操作在执行前必须用 `purgeTargetsRemainVisible`（`lib/workspace-visibility.ts`）重新校验目标——确认对话框执行的是打开时捕获的闭包，其间远端 pull 可能已退休父工作区。
