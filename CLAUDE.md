...
- **前端配额执行**（`store/plan-store.ts`）：`usePlanStore` 从 `GET /api/plan` 获取套餐限额和当前用量，缓存 5 分钟（TTL），落地到 `chrome.storage.local`（key `"tabslate-plan"`）。所有 create 类 store action 开头执行标准三步：`planStore.ensureFresh()` → `checkQuota(resource, localCount)` → 若超限则 `showQuotaAlert(resource)` + `return`；成功写入本地后调用 `incrementUsage(resource)`。delete 类 action 末尾调用 `decrementUsage(resource)`。**`checkQuota` 必须传入实际本地数量**（`localCount` 参数）——`fetchPlan` 会将 `usage` 重置为服务端计数，若本地有未同步的删除操作，服务端计数会比本地实际数量高，导致误判超限；传入本地 store 的真实数量可绕过此竞态。各资源本地数量：workspace = `get().workspaces.length`；collection = `get().collections.length`（含 active + archived + 软删除/垃圾箱中的集合；软删除后服务端 `is_deleted` 仍为 0，只有 `permanentlyDeleteCollection` 发送 `is_deleted:2` 后才从本地数组移除，因此 `collections.length` 与后端 `WHERE is_deleted = 0` 的计数天然一致）；tag = `get().tags.length`；bookmark = `get().bookmarks.length`；saved_group = `get().groups.filter(g => !g.deletedAt).length`。**集合配额 decrement 必须在 `permanentlyDeleteCollection` 上，而非 `deleteCollection`**——软删除后集合在服务端仍计入配额（`is_deleted = 0`），只有 `permanentlyDeleteCollection` 推送 `is_deleted: 2` 后才真正退出计数。`addTabToGroup` 在入口处对空 `groupId` 提前返回，防止配额拦截返回空字符串时产生孤儿 IDB 记录。
- **`StatsCards` 统计计数规则**（`components/dashboard/stats-cards.tsx`）：sidebar 下方的四格统计卡片直接从本地 store 派生计数，**必须与后端 `GET /api/plan` 的计算方式保持一致**：
  - **Collections**：`collections.filter(c => c.workspaceId === activeWorkspaceId).length`——只按工作区过滤，**不排除 archived 或 trashed**；与后端 `WHERE is_deleted = 0` 一致（软删除后 `is_deleted` 仍为 0，永久删除才从本地数组移除）。
  - **Bookmarks**：仍只统计 **active 集合**（`!deletedAt && !archivedAt`）下的活跃书签，用于展示当前可见书签数，非配额计数。
  - **Tags**：`tags.length`——`deleteTag` 直接从 state 移除，无软删除残留。
