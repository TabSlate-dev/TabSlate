...
- **前端配额执行**（`store/plan-store.ts`）：`usePlanStore` 从 `GET /api/plan` 获取套餐限额和当前用量，缓存 5 分钟（TTL）。所有 create 类 store action 开头执行标准三步：`planStore.ensureFresh()` → `checkQuota(resource)` → 若超限则 `showQuotaAlert(resource)` + `return`；成功写入本地后调用 `incrementUsage(resource)`。delete 类 action 末尾调用 `decrementUsage(resource)`。**集合配额 decrement 必须在 `permanentlyDeleteCollection` 上，而非 `deleteCollection`**——软删除后集合在服务端仍计入配额（`is_deleted = 0`），只有 `permanentlyDeleteCollection` 推送 `is_deleted: 2` 后才真正退出计数。`addTabToGroup` 在入口处对空 `groupId` 提前返回，防止配额拦截返回空字符串时产生孤儿 IDB 记录。
