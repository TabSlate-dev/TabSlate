...
- **groups 两态软删除**：groups 只有 active / trashed 两态，无 archived 状态。`deleteGroup` 写 `deletedAt = Date.now()`，保留 group 在 `groups` 数组（供回收站视图过滤 `groups.filter(g => !!g.deletedAt && g.workspaceId === activeWorkspaceId)`）并删除其所有 groupTabs；`restoreGroup` 清除 `deletedAt`，重置 `seq = 0` 并立即入队同步（服务端收到 `deleted_at: null` 后恢复记录）；`permanentlyDeleteGroup` 硬删除本地记录（IDB + state），不额外推送（服务端已持有软删除墓碑）。`TrashContent` 用 `useMemo(() => allGroups.filter(g => !!g.deletedAt && g.workspaceId === activeWorkspaceId), [allGroups, activeWorkspaceId])` 派生回收站列表，支持单条 / 批量 Restore + Delete Permanently + Empty Trash。渲染活跃 groups 的所有组件（sidebar、GroupsPanel）必须用 `!g.deletedAt && g.workspaceId === activeWorkspaceId` 过滤，避免其他工作区或软删除组显示。
