...
- **搜索 popup（`entrypoints/search/`）**：独立 WXT 入口，产物为 `search.html`；通过 background 的 `chrome.commands.onCommand("open-search")` 以 `chrome.windows.create({ type: "popup", width: 640, height: 420 })` 打开。`SearchApp` 等待 `useAuthStore._hydrated` 且 `loadTabs()` 完成后才渲染 `SearchPanel`。与 `BookmarksContent` 中的内联搜索共用同一 `SearchPanel` 组件，区别在于 popup 使用 `autoFocus + onClose={() => window.close()}`，内联使用 `smartOpen`。
