...
export interface WorkspacePullCheckpointDependencies {
  commit(operations: BulkWriteOp[]): Promise<void>;
...
export async function resolveAuthoritativeWorkspacePull(
...
): Promise<{ response: SyncPullResponse; capabilitySupported: boolean }> {
  ...
  return { response, capabilitySupported };
}
...
export async function commitWorkspacePullCheckpoint(
  ...
  const commit = dependencies?.commit ?? (async (pendingOperations: BulkWriteOp[]) => {
    const { idbBulkWrite } = await import("@/lib/idb");
    await idbBulkWrite(pendingOperations);
  });
...
async function loadAggregatePayload(
  workspaceId: string,
  ...
  const deferred = deferredRecord.payloadsByWorkspaceId[workspaceId] ?? createEmptySyncPushPayload();
