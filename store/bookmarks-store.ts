...
import {
  compareBookmarksNewestFirst,
  compareBookmarksTrashedFirst,
  normalizeFavicon,
...
function applySort(bookmarks: Bookmark[], sortBy: SortBy): Bookmark[] {
  ...
  switch (sortBy) {
    case "date-newest":
      return arr.sort(compareBookmarksNewestFirst);
    case "date-oldest":
      return arr.sort((a, b) => -compareBookmarksNewestFirst(a, b));
    case "alpha-az":
...
export const useBookmarksStore = create<BookmarksState>()(
  (set, get) => ({
      ...
      hydrate: async () => {
        ...
        set({
          bookmarks: map,
          countsByCollection: recomputeCounts(map),
          _hydrated: true,
        });
      },
      ...
      enqueueAllToSync: () => {
        void (async () => {
          ...
          syncEngine?.enqueue({
            bookmarks: [
              ...Array.from(current.bookmarks.values()).map((bookmark) => toServerBookmark(bookmark)),
      ...
      mergeFromServer: async (resp) => {
        ...
        set((current) => {
          ...
          for (const id of touchedIds) {
            if (nextBookmarks.delete(id)) {
              activeChanged = true;
            }
          }

          for (const bookmark of toActive) {
            if (current.bookmarks.get(bookmark.id) !== bookmark) {
              activeChanged = true;
            }
            nextBookmarks.set(bookmark.id, bookmark);
          }
      ...
      trashCollectionBookmarks: (collectionId) => {
        return (async () => {
          const state = get();
          const active = Array.from(state.bookmarks.values()).filter((bookmark) => bookmark.collectionId === collectionId);
      ...
      getTrashedBookmarks: () => {
        ...
        return applySearch([...state.trashedBookmarks], state.searchQuery)
          .sort(compareBookmarksTrashedFirst);
      },
    })
);
